...
import { USERS_TABLE_HEAD_EN, VEHICLE_TABLE_HEAD } from "../../app/constants/TableHeadings";
...
import { USERS, VEHICLES } from "../../config/endPoints";
import useSearchDebounce from "../../common/textDebounce/TextDebounce";
import ConfirmModal from "../../common/modal/ConfirmModal";
import CreateUserModal from "../users/section/customer/CreateUserModal";
import EditUserModal from "../users/section/customer/EditUserModal";
...
const VehicleManagement = () => {
  ...
  const vehicleListCallback = () => {
    getVehicle(
      `${VEHICLES.LIST}?page_no=${currentPage}&number_of_rows=${rowsPerPage}&role=${roleId}&search=${searchTextDebounce}`
    );
  };
  ...
  useEffect(() => {
    if (editUserId) {
      getVehicleDetails(`${USERS.DETAILS}/${editUserId}`);
    }
  }, [editUserId]);

  return (
    <>
      <PageMeta title={TITLES?.VEHICLES_MANAGEMENT.root} description="Dashboard" />

      <div className="flex justify-between items-center">
        <h1 className="font-semibold text-xl">{TITLES?.VEHICLES_MANAGEMENT.root}</h1>

        <div className="flex justify-end items-center">
          <Link
            to={ROUTES.VEHICLE_MANAGEMENT.Create_New_Driver}
      ...
      <div className="relative w-full overflow-x-auto hide-scrollba">
        <div className="min-w-[900px]">
          <table className="w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <TableHead headLable={VEHICLE_TABLE_HEAD} />
            <UserTableBody
              userList={userList}
              loading={loading}
