...
function FieldRow({ index, control, register, errors, onRemove }) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4 border p-3 mb-3 rounded-md relative bg-gray-50">
      ...
      {/* Placeholder */}
      <div>
        <label className="block text-sm font-medium mb-1">Placeholder (English)</label>
        <input
          type="text"
...
export default function UpdateStructure() {
  ...
  /* Fetch existing form structure and transform it */
  useEffect(() => {
    const fetchStructure = async () => {
      try {
        const res = await getRequest(`${FORMSTRUCTURE.EDIT}/${formID}`);
        if (res.data?.success) {
          const structureData = res.data.data;

          console.log(structureData , "structureData"); 
          
          // Set categories
          setValue("mainCategory", structureData?.category_id?.toString() || "");
          setValue("subCategory", structureData?.sub_category_id?.toString() || "");

          // Transform form_data from bilingual to English-only for editing
          if (structureData?.form_data?.length > 0) {
            const transformedFields = structureData.form_data.map(field => {
              // Extract English values from bilingual structure
              const baseField = {
                label: typeof field.label === 'object' ? field.label.en : field.label,
                name: field.name,
                type: field.type,
                placeholder: typeof field.placeholder === 'object' ? field.placeholder.en : field.placeholder,
                required: field.required,
                system: field.system || ["title", "description", "price", "price_syp"].includes(field.name),
                options: (field.options || []).map(opt => ({
                  value: typeof opt.value === 'object' ? opt.value.en : opt.value
                }))
              };
              return baseField;
            });
            ...
            const mergedFields = [
              ...systemFields.map(sys => 
                transformedFields.find(f => f.name === sys.name) || sys
              ),
              ...transformedFields.filter(f => 
                !["title", "description", "price", "price_syp"].includes(f.name)
              )
            ];
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      console.log(`${FORMSTRUCTURE.UPDATE}/${formID}`, formID);
      // console.log("Final payload being sent erere:", finalPayload);

      const res = await postRequest(`${FORMSTRUCTURE.UPDATE}/${formID}`, payload, {
        headers: {
      ...
      if (res.data?.success) {
        ...
        setOriginalFormData(data.fields);
        setShowPreview(true);
      } else {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      const res = await postRequest(`${FORMSTRUCTURE.UPDATE}/${formID}?isFormUpdated=true`, finalPayload, {
        headers: {
          "Content-Type": "application/json",
          "Accept-Language": "en",
        },
      ...
      if (res.data?.success) {
        toast.success("Form structure updated successfully!");
        navigate("/structure");
      } else {
    ...
    <>
      <form onSubmit={handleSubmit(handlePreview)} className="grid gap-6">
        ...
        <div className="text-right mt-4">
          ...
          >
            {loadingPreview ? "Generating Preview..." : "Preview & Update"}
          </button>
        </div>
      </form>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-3xl relative max-h-[85vh] overflow-y-auto">
                        ...
                        <div className="space-y-2">
                          {field.options?.map((option, i) => (
                            <label key={i} className="flex items-center gap-2">
                              <input type="radio" name={`preview-${field.name}`} />
                              <span>{option.value?.ar || option.value?.en}</span>
                            </label>
                          ))}
                        </div>
                      )}
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button
                ...
                className="primary-button"
              >
                {loadingSubmit ? "جاري التحديث..." : "Confirm & Update"}
              </button>
            </div>
          </div>
        </div>
