...
const SubCategoryTableBody = ({
  ...
  if (!loading && subCategoryList?.length === 0) {
    return (
      <DataNotFound
  ...
  return (
    <tbody>
      {loading ? (
        ...
        subCategoryList?.subCategories?.map((user, i) => {
          const { id, name, image_url, category } = user;
          ...
          return (
            <tr key={i} className="bg-white border-b text-sm border-gray-200">
              <td className="px-6 py-4">
                {
                  <img
                    src={image_url} // Using Avatar URL
                    alt={`${name}`}
