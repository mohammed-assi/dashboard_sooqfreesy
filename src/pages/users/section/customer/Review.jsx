...
const Review = () => {
  ...
  const [customerReviewData, setCustomerReviewData] = useSearchDebounce();
 ...
 async function getCustomerReview(url) {
  setLoading(true);
  ...
  const userListCallBack = () => {
    getCustomerReview(
      `${USERS.CUSTOMER_REVIEW}?customer_id=${id}&page=${currentPage}&limit=${rowsPerPage}&search=${searchTextDebounce}`
    );
  };
      ...
      <div className="relative w-full overflow-x-auto hide-scrollba">
        <div className="min-w-[900px]">
          <table className="w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <TableHead headLable={REVIEW_TABLE_HEAD} />
            <ReviewTableBody
              userList={userList}
              loading={loading}
