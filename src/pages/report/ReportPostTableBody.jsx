...
const ReportPostTableBody = ({
  ...
  if (!loading && postList?.reportListings?.length === 0) {
    return (
      <DataNotFound
        colSpan={REPORT_POST_TABLE_HEAD.length}
  ...
  return (
    <tbody>
      {loading ? (
        <TableSkeleton colSpan={REPORT_POST_TABLE_HEAD.length} rows={5} />
      ) : (
        (postList?.reportListings ?? []).map((post, i) => {
          const { reporter, listing, reportList, id, createdAt} =
            post;
          ...
          return (
            ...
            >
              ...
              <td className="px-6 py-4">{listing.title || "N/A"}</td>
              <td className="px-6 py-4">{reportList.name || "N/A"}</td>
              <td className="px-6 py-4">{moment(createdAt).format("MM/DD/YYYY")}</td>
              <td className="px-6 py-4">
                <div className="flex items-center justify-end">
                    <div className="group relative">
                    <Link to={`${ROUTES.USERS.detail}/${encodeId(listing?.id)}`}>
                      <i
...
export default ReportPostTableBody;
